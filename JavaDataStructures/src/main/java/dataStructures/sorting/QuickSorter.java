...
/**
 *  The general idea of quicksort:
 *       Divide and conquer the process of sorting an array by putting a single
 *      element in its place, then dividing the problem into sub-arrays around
 *      that pivot element.
 * 
 * @author Matt Crow
 */
public class QuickSorter {
    ...
    private void quickSort(Prioritizable[] array, int min, int max) {
        ...
        // but the index of the median value of a[min], a[mid], a[max - 1] is the best
        ...
        while(i <= j){ // exit when i and j cross. VERY IMPORTANT: i can equal j
            ...
            the pivot, that means the pivot is the largest value in
    ...
    public static void main(String[] args){
        ...
        for(int i = 0; i < unsorted.length; i++){
            unsorted[i] = new PrioritizableInteger(rng.nextInt(100));
        }
        
        System.out.println(Arrays.toString(unsorted));
        new QuickSorter().quickSort(unsorted);
        System.out.println(Arrays.toString(unsorted));
    }
}
